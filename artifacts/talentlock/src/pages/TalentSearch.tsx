...
import {
  useGetMe,
  useGetTalentSearch,
  useGetTalentSearchStats,
  useListTalentSearchActivity,
  useUpsertTalentSearch,
...
import { TalentSearchStatusBar } from "@/components/talent-search/TalentSearchStatusBar";
import { TalentSearchRuleBuilder } from "@/components/talent-search/TalentSearchRuleBuilder";
import { TalentSearchActivityFeed } from "@/components/talent-search/TalentSearchActivityFeed";
import { formatHours } from "@/lib/talentSearchDisplayUtils";

function TalentSearchStatsPanel({
  stats,
  isLoading,
}: {
  stats?: {
...
export default function TalentSearch() {
  ...
  const { data: config, isLoading: configLoading } = useGetTalentSearch({
    query: { enabled: isEmployer } as any,
  });
  const { data: stats, isLoading: statsLoading } = useGetTalentSearchStats({
    query: { enabled: isEmployer } as any,
  });
  const { data: activityMeta } = useListTalentSearchActivity(
    { page: 1, pageSize: 1 },
    { query: { enabled: isEmployer } as any },
  );

  const upsert = useUpsertTalentSearch();
  ...
  const handleSaveRules = async (rules: TalentSearchRules, rawRulesText?: string | null) => {
    await upsert.mutateAsync({ data: { rules, rawRulesText: rawRulesText ?? null } });
    await queryClient.invalidateQueries({ queryKey: getGetTalentSearchQueryKey() });
  };
    ...
    <div className="max-w-4xl mx-auto space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 border-b border-border/50 pb-6">
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm">
            Your AI assistant finds and reaches out to matching freelancers while you&apos;re away.
          </p>
        </div>
        <TalentSearchStatusBar config={config} stats={stats} hasConfig={!!config} />
      </div>
      ...
      <Tabs value={tab} onValueChange={setTab}>
        ...
        <TabsContent value="activity" className="mt-6">
          <TalentSearchActivityFeed />
        </TabsContent>

        <TabsContent value="stats" className="mt-6">
          <TalentSearchStatsPanel stats={stats} isLoading={statsLoading} />
        </TabsContent>
      </Tabs>
    </div>
  );
