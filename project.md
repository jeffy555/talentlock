...
| `agreements` | AI-generated or employer-uploaded legal agreements. Has signature fields, `status`, `healthScore`, `healthScoreDetail`, `healthScoredAt`, `freelancerSummary`, `freelancerSummaryScoredAt`, `source` (`ai_generated`\|`employer_upload`), `employerSummary`, `employerSummaryScoredAt`, `amendments`, `uploadFilename`, `uploadStage`, `finalizedAt`. |
...
GET  /api/job-requirements                        Paginated list (?page, ?pageSize, ?status, ?q, ?employerId — list UI default pageSize 10 — `spec/engagement-lists/`). Signed-in **freelancers** are hard-filtered to their onboarded `professionCategory` (`spec/domain-job-visibility/`). Employers / anonymous are not domain-scoped.
POST /api/job-requirements                        Create job
GET  /api/job-requirements/:id                    Job detail (freelancer: 404 if job domain ≠ profile domain)
...
PATCH /api/agreements/:id/amendments              Update employer amendment points (employer-uploaded agreements only)
...
1. **Dual Role System** — Registration requires a valid primary email address. Employers complete four steps (account type → location → company profile → mandatory Representative ID upload) before `PUT /api/users/me` sets `role: employer`. Freelancers complete role → profession category (if applicable) → location → profile details.
...
50. **Domain Job Visibility** — onboarded freelancers only see / match jobs in their `professionCategory` (technology, education, healthcare, legal_finance). Job list + detail + express-interest are server-filtered; Cruise Mode skips other-domain jobs before AI (no activity row). Employers and anonymous list callers are unchanged. Spec: `spec/domain-job-visibility/` (✅ Complete).
...
- TalentSearch fires on `PATCH /api/freelancers/me`, `POST /api/freelancers` (profile create), and freelancer document verification (AI or admin) when `completenessScore >= 60` — fire-and-forget, never awaited, never delays response
- Activating or dry-running TalentSearch (`PATCH /api/talent-search/activate|dry-run`) backfills evaluation against existing Talent-Vault-visible freelancers (completeness >= 60); backfill logs `prefilter_rejected` rows when rules exclude a profile
...
- Duplicate prevention: skip re-contacting the same freelancer within 30 days of a prior `sent` decision (per `employerId` + `freelancerId`); logs `duplicate_skipped` in activity feed
...
- Admin freelancer document preview in Document Review tab uses same-origin `/api/storage/objects/{fileUrl}` (admin session) — do not use raw signed URLs in `<img>` tags
- Admin document new-tab open must use `openStorageDocument()` (or equivalent): open `about:blank` **without** `noopener`, set `location.href`, then clear `opener` — `window.open(..., "noopener,noreferrer")` returns `null` and leaves a blank tab
- Admin `GET /api/admin/documents` includes `signedFileUrl`; optional `GET /api/admin/documents/:id/view-url` returns a fresh signed URL for fallback opens
...
- AI matching prompt: `buildProfessionContext()` MUST return `''` (empty string, no whitespace) for `professionCategory !== 'education'` — this guarantees byte-identical prompts for all existing technology jobs
...
- `freelancer_profiles.countryCode` + `currencyCode` are denormalised from `users` on location update — keep in sync via `syncFreelancerLocationFromUser()`
...
- `agreements.source = 'employer_upload'` distinguishes uploaded agreements from AI-generated ones
- Upload workflow stages: `summary_ready` → `enriched` → `finalized` via `uploadStage` column
- Employer cannot sign until `uploadStage === 'finalized'` — enforced in `POST /api/agreements/:id/sign`
- `employerSummary` is stripped from API responses for non-employers in `enrichAgreementForViewer()`
- Text extraction via `documentTextExtract.ts` — PDF (pdf-parse), DOCX/DOC (mammoth), TXT (utf-8)
- Token features: `agreement_upload_summary` (on upload-confirm), `agreement_upload_enrich` (on enrich)
- Storage path pattern: `uploads/{employerId}/agreements/{uuid}.{ext}` — validate ownership before confirm
...
| `artifacts/api-server/src/lib/documentTextExtract.ts` | `extractDocumentText()`, `extractTextFromBuffer()`, `isAllowedAgreementMimeType()` — PDF/DOCX/TXT text extraction |
| `artifacts/api-server/src/lib/employerAgreementSummaryUtils.ts` | `buildEmployerSummaryPrompt()`, `validateEmployerSummaryResponse()`, `EMPLOYER_AGREEMENT_SUMMARY_DISCLAIMER` |
| `artifacts/api-server/src/lib/agreementEnrichUtils.ts` | `buildEnrichPrompt()` — AI prompt for merging amendments + booking particulars into uploaded agreement |
| `artifacts/talentlock/src/components/agreements/EmployerAgreementUploadPanel.tsx` | Drag-and-drop upload UI on BookingDetail "Upload Yours" tab |
| `artifacts/talentlock/src/components/agreements/EmployerAgreementSummaryPanel.tsx` | Employer-facing AI summary display with attention flags |
| `artifacts/talentlock/src/components/agreements/EmployerAgreementWorkflow.tsx` | Multi-step stepper: Summary → Amendments → Enrich → Review → Sign |
...
- Intentionally public (no per-resource gate): `GET /api/freelancers`, `GET /api/freelancers/:id`, `GET /api/job-requirements`, `GET /api/job-requirements/:id`, public profile `/f/:id`, and `GET /api/storage/public-objects/*`. Signed-in **freelancers** still only receive jobs in their onboarded `professionCategory` on those job GET routes (`spec/domain-job-visibility/`).
...
- **Cruise Mode evaluation** — fires fire-and-forget after `POST /api/job-requirements`; use `evaluateCruiseModeForNewJob(db, jobId, req.log).catch(...)` — never awaited, never delays the response; evaluation failure must never affect job creation. Only evaluate freelancers whose `professionCategory` matches the job (`spec/domain-job-visibility/`); other-domain configs are skipped with no activity row.
