...
30. **Team Accounts (Enterprise)** — `employer_enterprise` plan; `teams` + `team_members` tables; invite flow with admin/member roles; shared team shortlist; team-level analytics; `requireTeamMember()` / `requireTeamAdmin()` guards on `/api/team/*`
...
33. **Agreement AI Summary** — Freelancer-only "✦ Summarise for me" on `/agreements/:id`; 6-section plain-English summary + up to 3 attention flags; disclaimer always first; cached on `freelancerSummary` / `freelancerSummaryScoredAt`; invalidated on redline accept; `agreement_summary` token label; 403 for employers
34. **Agreement PDF Download** — `GET /api/agreements/:id/download` returns a formatted PDF for fully signed agreements via `@react-pdf/renderer`; both signature images (or typed names), timestamps, metadata, TalentLock footer; cached in GCS after first generation; available to both parties on all plans; GDPR deletion removes cached PDFs
35. **Cruise Mode** — `freelancer_pro` feature; freelancer defines rules (skills, rate range, exclusions, blackout windows); two-stage pre-filter + AI evaluation on new job posts; personalised interest message to employer when score ≥ threshold; dry run mode; activity feed; monthly quota of 10 messages; employer sees "Cruise Mode ✦" badge; `cruise_mode_evaluation` token label
36. **Teaching Professional Profile** — `professionCategory` (`technology`|`education`) on `freelancer_profiles` and `job_requirements`; `rateType` on job posts; 12 nullable education fields for `educationProfessionType`; conditional onboarding step; Talent Vault `professionCategory`/`teachingSubject` filters; `formatRate()`/`rateUnitLabel()` centralises rate display
37. **TalentSearch (Employer Cruise Mode)** — mirror of Cruise Mode for employers; fires on `PUT /api/freelancers/me` when `profileMatchHash` changes; employer rules + AI evaluation; Express Interest notification to matching freelancers; per-`rulesVersion` duplicate prevention; freelancer opt-out; `talent_search_evaluation` token label; employer-only `/talent-search` page with teal accent
38. **AI Meeting Brief Generator** — fire-and-forget when meeting `status → confirmed`; 5-section employer brief (candidate snapshot, why they match, suggested questions, rate context, watch points); cached as `briefContent` on `meetings`; manual regen via `POST /api/meetings/:id/brief` (202); employer-only UI; Growth+ gates questions in UI only; `meeting_brief` token label
39. **In-App Direct Messaging** — `human_direct` conversations + `senderType`/`readAt` on messages; employer↔freelancer threads scoped to booking/meeting; fire-and-forget `new_message` notification + email; 30 messages/hour rate limit; `/messages` inbox + thread + booking/meeting tabs + freelancer profile Message button; unread nav badge; 30s polling; all plans
40. **Employer Verification** — `employer_documents` table (5 document types); `verificationLevel` / `isVerified` on `employer_profiles`; AI vision review with separate admin/employer notes; upsert on re-upload; `employer_doc_review` token label (tracked, not quota-deducted); admin Employer Docs tab; Verified Employer badge on jobs/bookings/meetings; aiNotes/fileUrl never returned to employers or freelancers
41. **Credential Expiry Tracking** — daily `POST /api/cron/credential-expiry` scan (GitHub Actions scheduled); tracks `documents.expiryDate` and `teachingLicenceExpiry`; staged alerts (90d/30d/7d/expired) via `expiryAlertStage`; Vault exclusion scoped to expired school-teacher teaching licences only; all plans, no token consumption
42. **Freelancer Watchlist** — employer personal pipeline on `saved_freelancers`; Watchlist tab on `/freelancers`; private notes; `WATCHLIST_UPDATE` notifications (availability + ≥5% rate change, debounced 24h); plan limits (starter 25 / growth 100); dashboard card; enterprise uses `team_shortlist` instead
43. **Post-Engagement AI Debrief** — fire-and-forget when booking `status → completed`; dual role-specific debrief in `debriefContent`; `GET/POST /api/bookings/:id/debrief`; 24h regen cooldown; `booking_debrief` token charged to employer; `BOOKING_DEBRIEF_READY` notification; violet/indigo `DebriefCard` on `/bookings/:id`; starter employer UI gating for sections 3–5
44. **Onboarding Scaffolding** — server-side `onboardingRole` / `onboardingStep` on `users`; `PATCH /users/me/onboarding-step`; resume onboarding on any device; freelancer `ProfileStrengthChecklist` on dashboard when completeness < 80%
